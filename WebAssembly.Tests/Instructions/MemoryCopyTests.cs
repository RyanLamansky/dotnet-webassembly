...
public class MemoryCopyTests
{
    ...
    {
        /// <summary>Copies len bytes from src to dst in memory, then returns the byte at result offset.</summary>
        public abstract int Test(int dst, int src, int len, int readOffset);
    }

    /// <summary>
    /// Tests that memory.copy moves bytes correctly without overlap.
    /// </summary>
    [TestMethod]
