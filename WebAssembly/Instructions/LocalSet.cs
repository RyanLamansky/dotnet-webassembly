...
public class LocalSet : VariableAccessInstruction
{
    ...
    internal sealed override void Compile(CompilationContext context)
    {
        ...
        else
        {
            ...
            Emit(context, localIndex);
        }
    }

    internal static void Emit(CompilationContext context, long localIndex)
    {
        switch (localIndex)
        {
            default:
                if (localIndex > 65534) // https://docs.microsoft.com/en-us/dotnet/api/system.reflection.emit.opcodes.stloc
                    throw new CompilerException(
                        $"Implementation limit exceeded: maximum accessible local index is 65534, tried to access {localIndex}.");
