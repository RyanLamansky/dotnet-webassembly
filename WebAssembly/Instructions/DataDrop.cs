...
/// <summary>
/// Drop a passive data segment, freeing its memory. Stack: [] → []
/// </summary>
public class DataDrop : MiscellaneousInstruction
{
    ...
    internal sealed override void Compile(CompilationContext context)
    {
        if (!context.DataSegments.TryGetValue(SegmentIndex, out var segField))
            throw new CompilerException($"data.drop: data segment {SegmentIndex} is not a passive segment or does not exist.");

        // Null out the field so memory.init on a dropped segment traps.
